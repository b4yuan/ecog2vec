...
def custom_sort_key(filename):
    num_part = int(filename.split('nwb_')[1].split('.wav')[0])
    return num_part
...
def get_example_dicts(subject, blocks, ecog_representation_path):
    ...
    for block in blocks:
        ...
        for index, trial in enumerate(nwbfile.trials):
            ...
            c, _ = sf.read(c_filepath)
            c = c.reshape(480,-1) # [nchannel, samples]
...
def get_decoding_inputs_and_targets(all_example_dicts):
    ...
    c_vectors = [torch.tensor(c_vectors[i]).reshape(1,-1,480) for i in range(len(c_vectors))]
    ...
    c_vectors = [c_vector.reshape(-1, 480) for c_vector in c_vectors]
...
subject = '401'
ecog_representation_path = '/home/bayuan/Documents/fall23/ecog2vec/wav2vec_outputs/nwbhg'
...
rnn = DynamicRNN(input_size, hidden_size, output_size)
rnn = rnn.to(DEVICE)
...
for epoch in range(num_epochs):
    ...
    for ecog_seq, target_seq in train_data_loader:
        ...
        # Forward pass through the DynamicRNN
